        ...
        for i in range(1, len(tokens)):  # -- Only process operand positions first filter the garbages
            ...
            if op.isdigit():  # Decimal
                ...
                if op in label_address:
                    ...
                    # if not ((instr in ["b", "call"] and label_address[op] & 0b1111 == address & 0b1111) or (instr not in ["b", "call"] and label_address[op] & 0b11111 == address & 0b11111)):
                    #     print("FAIL", instr, address, tokens)
    ...
    if label is not None:
        ...
        if label in pending_labels:
            while len(pending_labels[label]) > 0:
                ...
                # if not ((other_tokens[0] in ["b", "call"] and label_address[label] & 0b1111 == other_address & 0b1111) or (other_tokens[0] not in ["b", "call"] and label_address[label] & 0b11111 == other_address & 0b11111)):
